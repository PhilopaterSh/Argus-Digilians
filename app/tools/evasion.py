...
from app.tools.utils import normalize_domain_for_memory, SENSITIVE_CONTENT_INDICATORS, shell_quote
...
class EvasionService:
    ...
    def _with_param(url: str, param: str, value: str) -> str:
        """Merge `param=value` into `url`'s query string.

        The agent frequently hands this tool a URL that already carries a
        query - it reads a `?page=../../../etc/passwd` example out of
        Exploit_Suggester's PayloadsAllTheThings output and passes that
        straight in. Blindly appending `?{param}={value}` then produced a URL
        with two `?`, e.g.

            https://lab.net/?page=../../../etc/passwd?item=../../etc/passwd

        where everything after the second `?` is just part of the first
        parameter's value, so the traversal was never actually tested. That
        is exactly how a genuinely vulnerable PortSwigger lab came back as
        "No vulnerabilities detected" three times in a row.

        Args:
            url (str): Target URL, with or without an existing query string.
            param (str): Query parameter name to inject into.
            value (str): Payload to assign to `param`.

        Returns:
            str: `url` with `param=value` appended using the correct
            separator (`?` when there is no query yet, otherwise `&`).
        """
    ...
    def advanced_vuln_probe(self, url):
        ...
        *what* came back (a WAF challenge page or a normal error page can
        ...
        Args:
            url (str): Target URL to probe. An `item=`/`id=` query parameter
                is merged in per payload - if `url` already carries a query
                string, the parameter is appended with `&`, never a second
                `?` (see `_with_param`).
        ...
        for p in traversal_payloads:
            # shell_quote: dotdotpwn.txt entries can carry single quotes, which
            # used to terminate this argument and leave the command
            # syntactically invalid - bash aborted with "unexpected EOF" and
            # curl never ran, so those payloads were silently never tested.
            cmd = f"curl -s --max-time 15 --connect-timeout 5 {shell_quote(self._with_param(url, 'item', p))}"
        ...
        for p in sqli_payloads:
            # shell_quote: SQLi payloads are quote-dense by definition - the
            # hardcoded `1'/**/OR/**/1=1/**/--` above broke the command outright
            # and was never actually sent until this was quoted.
            cmd = (
                f"curl -s --max-time 15 --connect-timeout 5 "
                f"-w '\\n%{{http_code}}' {shell_quote(self._with_param(url, 'id', p))}"
            )
