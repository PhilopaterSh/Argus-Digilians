...
- [x] T016 Update `Setup/README.md` to point users to `scripts/ARGUS_INSTALLER.ps1` as the primary installer
...
- [ ] T018 ~~Copy `scripts/ARGUS_INSTALLER.ps1` from `Argus` to `remote_Argus_PhilopaterSh`~~ — superseded: the dual-clone layout was consolidated into this single repository, so there is no second directory to sync
- [ ] T019 ~~Verify identical hashes for `ARGUS_INSTALLER.ps1` across both directories~~ — superseded, see T018
- [ ] T020 Commit and push the merged branch to GitHub
