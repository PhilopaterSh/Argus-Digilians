@echo off
setlocal enabledelayedexpansion
cd /d "%~dp0"
title Argus System Health Check
color 0E

echo ========================================================
echo        (o) ARGUS SYSTEM HEALTH DIAGNOSTICS
echo ========================================================
echo.

set HEALTHY=YES

:: 1. Check Python & Venv
echo [*] Checking Python Environment...
if exist "Argus_venv\Scripts\python.exe" (
    echo [OK] Virtual Environment found at Root.
) else (
    echo [ERR] Virtual Environment ^(Argus_venv^) MISSING at Root.
    set HEALTHY=NO
)

:: 2. Check Ollama
echo [*] Checking Ollama Engine...
:: Checking for the process without relying on exact tasklist filtering which can be flaky
tasklist | findstr /I "ollama" >NUL
if %errorlevel% neq 0 (
    echo [ERR] Ollama process is NOT running.
    set HEALTHY=NO
) else (
    echo [OK] Ollama Engine is ONLINE.
)

:: 3. Check WSL & Kali
echo [*] Checking WSL (Kali Linux)...
wsl -d kali-linux -- echo ok >NUL 2>&1
if %errorlevel% neq 0 (
    echo [ERR] Kali Linux distro is NOT detected or NOT functional.
    set HEALTHY=NO
) else (
    echo [OK] Kali Linux is detected and functional.
)

:: 4. Check Embedding Model (nomic-embed-text)
echo [*] Checking Embedding Model (nomic-embed-text)...
ollama list | findstr /I "nomic-embed-text" >NUL
if %errorlevel% neq 0 (
    echo [WARN] nomic-embed-text NOT found. Run: ollama pull nomic-embed-text
) else (
    echo [OK] Embedding model is available.
)

:: 5. Check SSH Bridge (New Check)
echo [*] Checking SSH Bridge to WSL...
:: Auto-fix WSL SSH daemon by creating missing run dir and starting it
wsl -d kali-linux -u root bash -c "mkdir -p /run/sshd && /usr/sbin/sshd" >NUL 2>&1
powershell -Command "$res = Test-NetConnection -ComputerName 127.0.0.1 -Port 22 -ErrorAction SilentlyContinue; if ($res.TcpTestSucceeded) { exit 0 } else { exit 1 }" >NUL 2>&1
if %errorlevel% neq 0 (
    echo [ERR] SSH Bridge ^(Port 22^) is NOT accessible.
    set HEALTHY=NO
) else (
    echo [OK] SSH Bridge is ACTIVE.
)

echo.
echo --------------------------------------------------------
if "%HEALTHY%"=="YES" color 0A
if "%HEALTHY%"=="YES" echo [RESULT] SYSTEM IS HEALTHY AND READY!
if "%HEALTHY%"=="NO"  color 0C
if "%HEALTHY%"=="NO"  echo [RESULT] SYSTEM HAS ISSUES. PLEASE RUN INSTALL_EVERYTHING.bat
echo --------------------------------------------------------
if "%ARGUS_SKIP_PAUSE%"=="" pause
