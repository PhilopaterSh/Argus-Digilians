...
function Invoke-StepKaliTools {
    ...
    try {
        # Write the embedded script to /tmp/ inside WSL
        Write-Info "Writing embedded Kali installer to /tmp/argus_check_and_install.sh inside WSL..."
        wsl -u root bash -c "cat > /tmp/argus_check_and_install.sh << 'EMBEDDEDEOF'
$EMBEDDED_CHECK_INSTALL_SH
EMBEDDEDEOF" 2>&1 | ForEach-Object { Write-Info $_ }

        Write-Info "Normalizing line endings..."
        wsl -u root bash -lc "sed -i 's/\r$//' /tmp/argus_check_and_install.sh" 2>&1 | ForEach-Object { Write-Info $_ }

        Write-Info "Making script executable..."
        wsl -u root bash -lc "chmod +x /tmp/argus_check_and_install.sh" 2>&1 | ForEach-Object { Write-Info $_ }

        Write-Info "Running Kali tool installer..."
        wsl -u root bash -lc "bash /tmp/argus_check_and_install.sh" 2>&1 | ForEach-Object { Write-Info $_ }
