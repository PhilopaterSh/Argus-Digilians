...
| 025 Subtask-level benchmark suite | **Implemented and live-verified (2026-07-23)** - baseline SR 0/4, mean SCR 0.33 across 4 fixtures; ablation shows `019`'s reflection helping directionally (mean SCR 0.33 vs. 0.25 without it) | `benchmarks/fixture_base.py`, `benchmarks/runner.py`, `benchmarks/fixtures/{info_disclosure_env_leak,xss_reflected,idor_object_access,ssti_template_injection}/`, `benchmarks/results/*.md` | `benchmarks/tests/test_runner.py` (10 tests) | Fixed the gap in the migrated `tests/manual/ai_benchmark.py` (hand-picked 2-tool subset -> real `build_argus_tools()`) and a real, previously-undiscovered WSL-networking bug found via a live sanity run (fixture servers on `127.0.0.1` were unreachable from the WSL/Kali guest) - see `specs/025-subtask-benchmark-suite/`, `specs/checklist.md` CHK113 |
