...
| 018 Structured agent reliability | Implemented | `app/core/agent/brain.py` (rewired), `app/core/agent/react_workflow.py` (+structured final-answer, +max_iterations bug fix), `app/core/agent/react_callback.py` (+on_graph_event) | `tests/test_registry/test_brain_ask.py`, `tests/test_registry/test_react_callback.py`, `tests/test_langgraph_workflow.py` | Fixes a live 900s-timeout/zero-results production failure; `app/core/prompts.py` (017's original prompt) is no longer what drives tool selection - `ArgusBrain` now uses `react_workflow.py`'s shorter `react_prompts.py` + structured decoding for reliability; `app/core/prompts.py` remains used by `agent_factory.py`'s classic executor, kept for other callers |
