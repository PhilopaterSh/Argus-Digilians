...
| 025 Subtask-level benchmark suite | **Implemented (2026-07-23)** - T005/T009 (full baseline/ablation live runs) unblocked, left for a human-scheduled run | `benchmarks/fixture_base.py`, `benchmarks/runner.py`, `benchmarks/fixtures/{info_disclosure_env_leak,xss_reflected,idor_object_access,ssti_template_injection}/` | `benchmarks/tests/test_runner.py` (10 tests) | Fixed the gap in the migrated `tests/manual/ai_benchmark.py` (hand-picked 2-tool subset -> real `build_argus_tools()`) and a real, previously-undiscovered WSL-networking bug found via a live sanity run (fixture servers on `127.0.0.1` were unreachable from the WSL/Kali guest) - see `specs/025-subtask-benchmark-suite/`, `specs/checklist.md` CHK113 |
