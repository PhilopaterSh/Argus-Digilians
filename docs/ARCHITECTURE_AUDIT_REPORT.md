...
| 020 Multi-agent role separation | **Proposed, flagged high-risk/optional** (spec kit only, 2026-07-10) | not started | not started | Depends on 019; explicitly recommended to defer until 019's measured residual gap justifies the added architectural risk - see `specs/020-multi-agent-role-separation/spec.md`'s "Why this feature (and why it is flagged high-risk)" |
...
spec -> code -> tests. `020`-`026` remain **intentionally** spec-kit-only (a backlog produced
from a 2026-07-10 gap analysis against `docs/history/2603.27127v1.pdf`, at the user's request) -
not implementation gaps, since no code was ever claimed for them; `019` (same backlog) has since
been implemented, see its row above. See `specs/checklist.md`'s "Backlog - Proposed Future
