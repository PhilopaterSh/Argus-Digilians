...
| 025 Subtask-level benchmark suite | **Implemented and live-verified (2026-07-23)** - 5 fixtures, baseline SR 0/5, mean SCR 0.33; two ablation runs disagreed (0.33 vs. 0.25, then 0.33 vs. 0.33 after adding a 5th fixture) - `enable_inter_reflection`'s effect not yet distinguishable from run-to-run variance at this scale, reported as such rather than only the first, more flattering run | `benchmarks/fixture_base.py`, `benchmarks/runner.py`, `benchmarks/fixtures/{info_disclosure_env_leak,xss_reflected,idor_object_access,ssti_template_injection,path_traversal_download}/`, `benchmarks/results/*.md` | `benchmarks/tests/test_runner.py` (10 tests) | Fixed the gap in the migrated `tests/manual/ai_benchmark.py` (hand-picked 2-tool subset -> real `build_argus_tools()`) and a real, previously-undiscovered WSL-networking bug found via a live sanity run (fixture servers on `127.0.0.1` were unreachable from the WSL/Kali guest) - see `specs/025-subtask-benchmark-suite/`, `specs/checklist.md` CHK113 |
...
above), see their rows above. See `specs/checklist.md`'s "Backlog - Proposed Future
Phases" section for sequencing recommendations on the rest.
