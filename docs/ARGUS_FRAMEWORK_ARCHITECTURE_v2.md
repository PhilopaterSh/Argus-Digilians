...
- **ADR 22: Wire `Query_Knowledge_Graph`'s data source into the live ReAct path (2026-07-24).** Found during a live-code investigation (prompted by a user question about using `graphify` for target-relationship mapping - see `graphify`'s own dev-only scoping under Principle XII, which ruled it out for this): `Query_Knowledge_Graph` (`brain_tools.py`, description "find cross-target relationships, shared infrastructure, and lateral movement paths") was a fully-registered agent tool with no data ever behind it in production. `003`'s `entities`/`relations` tables (`upsert_entity`/`add_relation`, `get_graph_insights()`) were only ever written by `ArgusBrain.run_deterministic_recon()`'s `SUBDOMAIN_OF`/`USES_TECH` edge logic, reachable exclusively via `ask_deterministic()` - itself reachable only when a caller passes `on_phase=` to `ask()`. A full-repo grep for such a caller found none: not the Tkinter GUI (`desktop_gui.py` calls plain `brain.ask()`), not `benchmarks/runner.py` (`brain.ask(query, callbacks=[trace_cb])`), not the Streamlit dashboard. Every live agent run's `Query_Knowledge_Graph` call returned the hardcoded `"No cross-target relationships recorded yet."` regardless of what was actually discovered. Fix: extracted the parsing/edge-writing helpers (`to_bare_hostname`/`parse_subdomains`/`parse_tech_block`/`clean_tech_string`/`record_graph_edge`) out of `brain.py` into `app/tools/utils.py` as a single source of truth (Constitution IX) - `brain.py`'s own methods now delegate rather than duplicate - and wired the same edge writes into `react_workflow.py`'s `execute_node` (the actual production single-loop path) and `_run_specialist_step` (the `020` experimental multi-role path), firing right after a `Subdomain_Enumeration`/`Recon_Suite` call succeeds. `VULNERABLE_TO` edges deliberately NOT added in this pass - the existing vulnerability signal (`_extract_vulnerability_hints`) is a heuristic hint (keyword/title match), not a confirmed finding, and turning it into a graph edge would overstate confidence beyond what was actually verified (Constitution VIII - Truthful Runtime); left as a documented, explicit follow-up, not a silent gap. Verified: CI's exact `mypy` scope, `ruff`, `check_docstrings.py`, `validate_ascii.py`, `check_duplication.py --diff origin/main` (zero new violations), and the full 381-test suite all pass.
...
Research provenance gate for this pass even though the outcome is still pending). Updated again
2026-07-24: added ADR-22, documenting the same day's fix wiring `Query_Knowledge_Graph`'s
`entities`/`relations` data source into the live ReAct path (`react_workflow.py`) - previously only
an unreachable deterministic-pipeline path ever populated it. `specs/003-sqlite-blackboard/
data-model.md`'s Entity 3/4 descriptions updated to match in the same pass.*
